...
class ErniePrompt(nn.Module, ABC):
    def __init__(self, opt):
        super(ErniePrompt, self).__init__()
        self.model_name = 'ErnieModel'
...
class Model(nn.Module):
    def __init__(self,opt):
        ...
        self.ernie_layer = ErniePrompt(opt)
    ...
    def forward(self,sentence,mask,prompt_mask_idx):
        ernie_vec,ernie_prompt_vec = self.ernie_layer(sentence,mask,prompt_mask_idx) # [batch_size, hidden_size]
        hidden_vec = torch.relu(self.cls_layer1(ernie_vec))
        y_hat = self.cls_layer2(hidden_vec)

        hidden_vec = torch.relu(self.prompt_cls_layer1(ernie_prompt_vec))
