...
class DatasetTrain(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mask_idx = len(item['doc']['input_ids'])
        doc_idx = item['doc']['input_ids'] + [self.tokenizer.mask_token_id] + prompt['input_ids'] +  [self.tokenizer.sep_token_id]
        doc_mask = item['doc']['attention_mask'] + [1] + prompt['attention_mask'] + [1]
    ...
    def _preprocess(self):
        ...
        for dic in self.dicts:
            ...
            if len(item['nodes'] ) != self.class_num:
                ...
                item['adj_matrix'] = np.zeros((len(item['nodes']),len(item['nodes'])))
            if len(dic['path_T']) + len(dic['path_F'])==0:
                continue
            label2path_t = {}
    ...
    def __getitem__(self, idx):
        ...
        for label in path_labels:
            if label in item['pos_path']:
                prompt = self.id2path[random.choice(item['pos_path'][label])].replace('\t','')
                positive = 1
            else:
                prompt =self.id2path[random.choice(item['neg_path'][label])].replace('\t','')
                positive = 0
            prompt = self.tokenizer(''.join(prompt),add_special_tokens = False)
            ...
            mask_idx = len(raw_doc)
            mask_idx = np.array([mask_idx])


            doc_idx = raw_doc + [self.tokenizer.mask_token_id] + prompt['input_ids'] +  [self.tokenizer.sep_token_id]
            doc_mask = raw_mask + [1] + prompt['attention_mask'] + [1]
